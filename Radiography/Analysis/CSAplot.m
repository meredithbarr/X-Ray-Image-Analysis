...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[]=CSAplot(data,figtitle,incscans,leglabels,legtitle)
...
for i=index
    loop=loop+1;
    plot(data(i).t,data(i).CSA,'Color',color(loop,:),'LineStyle',string(style(loop)))
    hold on;
end

%axis and axis labels
%title(figtitle);
ylabel({'Percent change in bed CSA (%)'});
xlabel('Time (min)');
axis tight
hold on
...
%save figure
figname=sprintf('%s_CSA',figtitle);
savefig(figname);

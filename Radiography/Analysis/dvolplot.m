...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[]=dvolplot(data,figtitle,incscans,leglabels,legtitle)
...
for i=index
    loop=loop+1;
    plot(data(i).t,data(i).dvol,'Color',color(loop,:),'LineStyle',string(style(loop)))
    hold on;
end

%axis and axis labels
%title(figtitle);
ylabel({'Rate of estimted bed volume change (%/min)'});
xlabel('Time (min)');
axis tight
hold on
...
%save figure
figname=sprintf('%s_dvol',figtitle);
savefig(figname);

...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[]=dAPVplot(data,figtitle,incscans,leglabels,legtitle)
...
for i=index
    loop=loop+1;
    plot(data(i).t,data(i).dAPV,'Color',color(loop,:),'LineStyle',string(style(loop)))
    hold on;
end

%axis and axis labels
%title(figtitle);
ylabel({'Rate of normalised bed APV change (%/min)'});
xlabel('Time (min)');
axis tight
hold on
...
%save figure
figname=sprintf('%s_dAPV',figtitle);
savefig(figname);
